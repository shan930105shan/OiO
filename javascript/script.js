...
document.getElementById("langSwitch").addEventListener("change", function() {
    ...
    let selectedLang = this.checked ? "en" : "zh";  // 選擇語言

    langText.textContent = selectedLang === "en" ? "EN" : "中"; //更新文字
...
function switchLanguage(lang) {
    const translations = {
        "en": {
            ...
            "other": "Other",
            //首頁
            "OiO-image":"img/OiO-EN.png",
            "Home-page":"img/首頁圖-EN.png",
        ...
        "zh": {
            ...
            "other": "其他",
            //首頁
            "OiO-image":"img/OiO.png",
            "Home-page":"img/首頁圖.png",
    ...
    // 切換文本和图片，更新 hover-block 的 data-image 属性但不改 OiO-introduce-img
    ...
    // 更新 hover-block 中的 data-image 属性，但不改 OiO-introduce-img-的 src
...
// 添加點擊事件，切换 mainImage 的路径
document.querySelectorAll(".introduce-img").forEach(img => {
    img.addEventListener("click", function() {
        const lang = getCurrentLanguage(); // 當前語言的變量或函數
...
function updateSlider() {
  for (let i = 0; i < images_rec.length; i++) {
    ...
    // 根據索引顯示或隱藏圖片
    if (i >= 4) {
      images_rec[i].style.display = 'none'; // 
    } else {
